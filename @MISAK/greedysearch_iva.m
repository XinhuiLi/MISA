...
%         O.update(S_,M,b',l',e');
        ...
        for cc = randperm(size(S{mm}, 2))%1:size(S_{mm}, 2)%
            ...
            kk = find(S_{mm}(current,:));             % all sources in current subspace
            ...
            O.update(S_,M,[b'; b(end)],[l'; l(end)],[e'; e(end)]); % Update S and set M = mm
...
%         figure, imagesc(full(S_{mm}),[0 1]), axis equal tight; title('AFTER subspace match: ESTIMATED')
        ...
        tmpW = {};
        for mm_ = M
            oldW{mm_} = oldW{mm_}(shuff,:);
            tmpW{mm_} = O.W{mm_}(shuff,:);
        end
        O.objective(O.ut.stackW(tmpW));
    end
