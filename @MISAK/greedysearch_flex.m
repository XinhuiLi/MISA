...
%         figure
        for cc = randperm(size(S{mm}, 2))%1:size(S_{mm}, 2)%
            ...
            O.update(S_,mm,b,[],e); % Update S and set M = mm
...
%     O.update(S,M,old_beta,old_lambda,old_eta);
    O.update(S,M,old_beta(1),[],old_eta(1));

%     O.updateRElambda(REl);
%     [~,shuff] = O.sub_perm_analysis(O.ut.stackW(O.W));

    Sm = mat2cell(blkdiag(O.S{:}),O.K*length(M),O.C); % disconnect subspaces across modalities
    O.update(Sm,M,old_beta(1),[],old_eta(1));
    
    shuff = O.greedy_sub_perm_analysis();
