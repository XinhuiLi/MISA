...
figure,
subplot(2,2,1);imagesc(sim_misa.S{1},max(max(abs(sim_misa.S{1}))).*[-1 1]);colorbar();title("Ground Truth S Modality 1");
subplot(2,2,2);imagesc(sim_misa.S{end},max(max(abs(sim_misa.S{end}))).*[-1 1]);colorbar();title("Ground Truth S Modality 2");
subplot(2,2,3);imagesc(data2.S{1},max(max(abs(data2.S{1}))).*[-1 1]);colorbar();title("Estimated S Modality 1");
subplot(2,2,4);imagesc(data2.S{end},max(max(abs(data2.S{end}))).*[-1 1]);colorbar();title("Estimated S Modality 2");
